...
pick_up_test_outputs <- function(cdir, edir){
  ...
  if (nchar(target) > 0) {
    if (file.exists(target)) {
      if(file.exists(cdir)){
        target_test <- file.path(target, "test", "testthat" ,get_after_root_in_testthat(edir))
        file.copy(list.files(cdir, full.names = TRUE), target, overwrite = TRUE, recursive = TRUE)
      }
    }
  }
}
